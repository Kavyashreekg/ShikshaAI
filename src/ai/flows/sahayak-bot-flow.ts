...
import {z} from 'zod';
import {instantKnowledgeExplanation} from './instant-knowledge-explanation';
import {generateLocalizedStory} from './generate-localized-story';
import {designVisualAid} from './design-visual-aid';

import {
  InstantKnowledgeExplanationInputSchema,
  InstantKnowledgeExplanationOutputSchema,
  type InstantKnowledgeExplanationInput,
  type InstantKnowledgeExplanationOutput,
} from './schemas/instant-knowledge-explanation.schema';
import {
  GenerateLocalizedStoryInputSchema,
  GenerateLocalizedStoryOutputSchema,
  type GenerateLocalizedStoryInput,
  type GenerateLocalizedStoryOutput,
} from './schemas/generate-localized-story.schema';
import {
  DesignVisualAidInputSchema,
  DesignVisualAidOutputSchema,
  type DesignVisualAidInput,
  type DesignVisualAidOutput,
} from './schemas/design-visual-aid.schema';
...
const explainConceptTool = ai.defineTool(
  {
    name: 'explainConcept',
    description:
      'Explains a concept in simple terms with an analogy. Use this for questions like "what is X?" or "explain Y".',
...
const createStoryTool = ai.defineTool(
  {
    name: 'createStory',
    description:
      'Creates a hyper-localized and culturally relevant story for children on a given topic and language.',
...
const createVisualAidTool = ai.defineTool(
  {
    ...
    description:
      'Generates a simple, clear visual aid (like a line drawing or chart) based on a description, suitable for a blackboard.',
...
const SahayakBotInputSchema = z.object({
  ...
  language: z
    .string()
    .describe('The language for the response, if applicable for the tool.'),
});
...
const SahayakBotOutputSchema = z.object({
  ...
  response: z
    .string()
    .describe(
      'A conversational response confirming the action taken or providing an answer if no specific tool was used.'
    ),
});
...
export const sahayakBotFlow = ai.defineFlow(
  {
    name: 'sahayakBotFlow',
    inputSchema: SahayakBotInputSchema,
    outputSchema: z.any(),
    stream: true,
  },
  async (input, stream) => {
    ...
    toolResults.forEach(result => {
      if (result.toolName === 'explainConcept') {
        finalOutput.explanation =
          result.output as InstantKnowledgeExplanationOutput;
